...
document.getElementById('formAddUser').addEventListener('submit', async (e) => {
  ...
  else { alert("User berhasil ditambah!"); document.getElementById('formAddUser').reset(); muatDataUser(); }
});
...
async function muatDataLaporan() {
  ...
  data.forEach(rep => {
    ...
    if(rep['D-K'] === 2) detail.push("<b>STATUS IZIN (Semua Kriteria=2)</b>");
      ...
      if(rep['T-K']!==null) detail.push(`Tilawah(${rep['T-K']})`);
      if(rep['T-TK']!==null) detail.push(`TilawahTdk(${rep['T-TK']})`);
      if(rep['T-M']!==null) detail.push(`Murottal(${rep['T-M']})`);
      if(rep['T-T']!==null) detail.push(`Terjemah(${rep['T-T']})`);
      if(rep['SD-D']!==null) detail.push(`Dhuha(${rep['SD-D']})`);
      if(rep['SD-H']!==null) detail.push(`DhuhaHaid(${rep['SD-H']})`);
      if(rep['ST-T']!==null) detail.push(`Tahajjud(${rep['ST-T']})`);
      if(rep['ST-H']!==null) detail.push(`TahajjudHaid(${rep['ST-H']})`);
      if(rep['SF-F']!==null) detail.push(`Fajar(${rep['SF-F']})`);
      if(rep['SF-H']!==null) detail.push(`FajarHaid(${rep['SF-H']})`);
      if(rep['D-K']!==null) detail.push(`Dzikir(${rep['D-K']})`);
      if(rep['D-PG']!==null) detail.push(`DzikirPagi(${rep['D-PG']})`);
      if(rep['D-PT']!==null) detail.push(`DzikirPetg(${rep['D-PT']})`);
    }
    ...
    listLaporan.innerHTML += `
      <div class="item-card">
        <div class="item-header">
          <span>${rep.user}</span>
          <span class="badge bg-hijau">Juz ${rep['T-J'] || '-'}</span>
        </div>
...
function siapkanEditLaporan(rep) {
  ...
  if (rep['D-K'] === 2) {
    document.getElementById('adminTilawah').value = '';
    document.getElementById('adminDhuha').value = '';
    document.getElementById('adminTahajjud').value = '';
    document.getElementById('adminFajar').value = '';
    document.getElementById('adminDzikir').value = 'IZIN';
  } else {
    if(rep['T-K']===100) document.getElementById('adminTilawah').value = 'T-K';
    else if(rep['T-TK']===0) document.getElementById('adminTilawah').value = 'T-TK';
    else if(rep['T-M']===100) document.getElementById('adminTilawah').value = 'T-M';
    else if(rep['T-T']===100) document.getElementById('adminTilawah').value = 'T-T';
    else document.getElementById('adminTilawah').value = '';

    if(rep['SD-D']===100) document.getElementById('adminDhuha').value = 'SD-D';
    else if(rep['SD-TD']===0) document.getElementById('adminDhuha').value = 'SD-TD';
    else if(rep['SD-H']===1) document.getElementById('adminDhuha').value = 'SD-H';
    else document.getElementById('adminDhuha').value = '';

    if(rep['ST-T']===100) document.getElementById('adminTahajjud').value = 'ST-T';
    else if(rep['ST-TT']===0) document.getElementById('adminTahajjud').value = 'ST-TT';
    else if(rep['ST-H']===1) document.getElementById('adminTahajjud').value = 'ST-H';
    else document.getElementById('adminTahajjud').value = '';

    if(rep['SF-F']===100) document.getElementById('adminFajar').value = 'SF-F';
    else if(rep['SF-TF']===0) document.getElementById('adminFajar').value = 'SF-TF';
    else if(rep['SF-H']===1) document.getElementById('adminFajar').value = 'SF-H';
    else document.getElementById('adminFajar').value = '';

    if(rep['D-K']===100) document.getElementById('adminDzikir').value = 'D-K';
    else if(rep['D-PG']===50) document.getElementById('adminDzikir').value = 'D-PG';
    else if(rep['D-PT']===50) document.getElementById('adminDzikir').value = 'D-PT';
    else if(rep['D-TK']===0) document.getElementById('adminDzikir').value = 'D-TK';
    else document.getElementById('adminDzikir').value = '';
  }
...
function batalEdit() {
  ...
  document.getElementById('judulFormLaporan').innerText = "Tambah Laporan Baru";
  document.getElementById('btnAdminSubmitLaporan').innerText = "Simpan Laporan";
...
document.getElementById('formAdminReport').addEventListener('submit', async (e) => {
  e.preventDefault();
  const idEdit = document.getElementById('editReportId').value;
  const userDipilih = document.getElementById('adminSelectUser').value;
  if(!userDipilih) { alert("Pilih user!"); return; }

  const valTJ = parseInt(document.getElementById('adminTJ').value) || null;
  const valTilawah = document.getElementById('adminTilawah').value;
  const valDhuha = document.getElementById('adminDhuha').value;
  const valTahajjud = document.getElementById('adminTahajjud').value;
  const valFajar = document.getElementById('adminFajar').value;
  const valDzikir = document.getElementById('adminDzikir').value;
  ...
  if (valDzikir === 'IZIN') {
    ...
    if (!idEdit) payload.user = userDipilih;
  } else {
    if (!idEdit) payload.user = userDipilih;
    if (valTJ !== null) payload['T-J'] = valTJ;

    ['T-K', 'T-TK', 'T-M', 'T-T'].forEach(c => payload[c] = null); 
    if (valTilawah === 'T-K') payload['T-K'] = 100; else if (valTilawah === 'T-TK') payload['T-TK'] = 0; else if (valTilawah === 'T-M') payload['T-M'] = 100; else if (valTilawah === 'T-T') payload['T-T'] = 100;

    ['SD-D', 'SD-TD', 'SD-H'].forEach(c => payload[c] = null);
    if (valDhuha === 'SD-D') payload['SD-D'] = 100; else if (valDhuha === 'SD-TD') payload['SD-TD'] = 0; else if (valDhuha === 'SD-H') payload['SD-H'] = 1;

    ['ST-T', 'ST-TT', 'ST-H'].forEach(c => payload[c] = null);
    if (valTahajjud === 'ST-T') payload['ST-T'] = 100; else if (valTahajjud === 'ST-TT') payload['ST-TT'] = 0; else if (valTahajjud === 'ST-H') payload['ST-H'] = 1;

    ['SF-F', 'SF-TF', 'SF-H'].forEach(c => payload[c] = null);
    if (valFajar === 'SF-F') payload['SF-F'] = 100; else if (valFajar === 'SF-TF') payload['SF-TF'] = 0; else if (valFajar === 'SF-H') payload['SF-H'] = 1;

    ['D-K', 'D-PG', 'D-PT', 'D-TK'].forEach(c => payload[c] = null);
    if (valDzikir === 'D-K') payload['D-K'] = 100; else if (valDzikir === 'D-PG') payload['D-PG'] = 50; else if (valDzikir === 'D-PT') payload['D-PT'] = 50; else if (valDzikir === 'D-TK') payload['D-TK'] = 0;
  }

  if (idEdit) {
    const { error } = await supabaseClient.from('table_data_report').update(payload).eq('id', idEdit);
    if (error) alert("Gagal update: " + error.message); else { alert("Sukses update!"); batalEdit(); muatDataLaporan(); }
  } else {
    const { error } = await supabaseClient.from('table_data_report').insert([payload]);
    if (error) alert("Gagal simpan: " + error.message); else { alert("Sukses tambah!"); batalEdit(); muatDataLaporan(); }
  }
});
...
function getIkonRekap(rep) {
  ...
  let ikon = [];
  if (rep['T-K'] === 100) ikon.push('✅');
  else if (rep['T-M'] === 100) ikon.push('🎧');
  else if (rep['T-T'] === 100) ikon.push('🔀');
  else if (rep['T-TK'] === 0) ikon.push('💔');
  else ikon.push('➖');

  if (rep['SD-D'] === 100) ikon.push('✅');
  else if (rep['SD-TD'] === 0) ikon.push('💔');
  else if (rep['SD-H'] === 1) ikon.push('🚺');
  else ikon.push('➖');

  if (rep['ST-T'] === 100) ikon.push('✅');
  else if (rep['ST-TT'] === 0) ikon.push('💔');
  else if (rep['ST-H'] === 1) ikon.push('🚺');
  else ikon.push('➖');

  if (rep['SF-F'] === 100) ikon.push('✅');
  else if (rep['SF-TF'] === 0) ikon.push('💔');
  else if (rep['SF-H'] === 1) ikon.push('🚺');
  else ikon.push('➖');

  if (rep['D-K'] === 100) ikon.push('✅');
  else if (rep['D-PG'] === 50 || rep['D-PT'] === 50) ikon.push('💗');
  else if (rep['D-TK'] === 0) ikon.push('💔');
  else ikon.push('➖');
...
async function muatRekapHarian() {
  ...
  if (teksRekapGlobal === "") {
    ...
    teksRekapGlobal = "Belum ada laporan amaliyah untuk sesi ini.";
  }
...
function copyRekap() {
  ...
  navigator.clipboard.writeText(teksRekapGlobal.trim()).then(() => {
    ...
    btnCopy.style.background = "var(--hijau)";
    
    setTimeout(() => {
      btnCopy.innerText = textAwal;
      btnCopy.style.background = "var(--biru)";
    }, 2000);
  }).catch(err => {
    alert('Browser tidak mendukung auto-copy. Silakan blok teks manual.');
  });
}
